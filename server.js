...
const BROWSER_LAUNCH_MAX_RETRIES = parseInt(
  process.env.BROWSER_LAUNCH_MAX_RETRIES || "3"
);
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt === maxRetries) {
        throw new Error(
          `Failed to launch browser after ${maxRetries} attempts: ${error.message}`
        );
      }
  ...
  async (request, reply) => {
    ...
    if (!isValidUrl(url)) {
      return reply.code(400).send({
        ...
        details:
          "URL must use HTTP/HTTPS protocol and cannot target private networks",
      });
    }
...
const start = async () => {
  try {
    ...
    console.log(
      `✓ Rate limit: ${RATE_LIMIT_MAX} requests per ${RATE_LIMIT_WINDOW}`
    );
  } catch (err) {
